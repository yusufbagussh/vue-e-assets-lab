          ...
          <div class="card-body col-8">
            <form @submit.prevent="update" enctype="multipart/form-data">
              ...
              <div class="form-gorup mb-2">
                ...
                <div v-else-if="item_gambar">
                  <img
                    v-bind:src="'http://localhost:8000/storage/' + item_gambar"
                    width="100"
                    height="100"
                  />
                </div>
...
export default {
  ...
  methods: {
    ...
    async getItemById() {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/item/${this.$route.params.id}`,
          {
            headers: { Authorization: "Bearer " + this.token },
          }
        );
        ...
        this.item_gambar = response.data.data[0].item_gambar;
        this.item_kondisi = response.data.data[0].item_kondisi;
        this.item_spesifikasi = response.data.data[0].item_spesifikasi;
        this.item_status = response.data.data[0].item_status;
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async update() {
      try {
        await axios
          .post(
            `http://localhost:8000/api/updateitem/${this.$route.params.id}`,
            {
