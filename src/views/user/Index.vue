...
export default {
  ...
  methods: {
    sortTable(sortBy) {
      ...
      axios
        .get(
          `http://localhost:8000/api/user?page=${this.current_page}&paginate=${this.paginate}&search=${this.search}&sortBy=${this.sortBy}&sortOrder=${this.sortOrder}`,
          {
    ...
    getResult() {
      axios
        .get(
          `http://localhost:8000/api/user?paginate=${this.paginate}&search=${this.search}`,
          {
            headers: { Authorization: "Bearer " + this.token },
          }
        )
